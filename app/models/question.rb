class Question < ApplicationRecord
    ...
    def related_questions()
        ...
        results = ActiveRecord::Base.connection.exec_query("select id, title from (select id, title, (" + words.split(" ").map{|w| "(case when LOWER(title) like '%#{w}%' then 1 else 0 end)"}.join("+") + ") as count from questions where id != #{self.id} order by count desc) as t where t.count > 0 limit 10")
        return results
    end
end
